    ...
    member this.CreateShader(compute: Expr<unit -> unit>) =
        ...
        let options =
            { 
                ...
                ExecutionModel = ExecutionModel.GLCompute
                ExecutionMode = Some(ExecutionMode.LocalSize(1u, 1u, 1u))
            }
        let compilation = FSharpSpirvQuotationCompilation.Create(options, compute)

        let vertexBytes =
            use ms = new System.IO.MemoryStream 100
            if not (compilation.Emit ms) then
                failwithf "%A" (compilation.GetDiagnostics())
            let bytes = Array.zeroCreate (int ms.Length)
            ms.Position <- 0L
            ms.Read(bytes, 0, bytes.Length) |> ignore
            bytes

        let vertexLayouts, vertexInputs = checkShaderInputs ComputeStage (compilation.GetVariables())
