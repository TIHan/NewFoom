[<AutoOpen>]
module Falkan.FalkanDeviceMemory
...
type FalkanDeviceMemory(bucket: DeviceMemoryBucket, offset: int, size: int) =
...
and [<Sealed>] DeviceMemoryBucket private (device: VkDevice, raw: VkDeviceMemory, bucketSize: int) as this =
    ...
    let allocateBlock size =
        let block =
            if Seq.isEmpty blocks then
                let block = FalkanDeviceMemory(this, 0, size)
                blocks.Add struct(block, false)
                block
            elif getFreeSize () >= size then
                let lastBlock = getLastBlock ()
                let block = FalkanDeviceMemory(this, lastBlock.Offset + lastBlock.Size, size)
                blocks.Add struct(block, false)
                block
            elif getMaxFreeFragmentSize () >= size then
                let struct(block, index) = getFreeFragmentBlock size
                let newBlock = FalkanDeviceMemory(this, block.Offset, size)
                ...
                if remainingSize > 0 && remainingIndex < blocks.Count then
                    let remainingBlock = FalkanDeviceMemory(this, newBlock.Offset + newBlock.Size, remainingSize)
    ...
    let freeBlock (block: FalkanDeviceMemory) =
        ...
        if possibleNextBlockIndexToMerge < blocks.Count && isBlockFree possibleNextBlockIndexToMerge then
            ...
            blocks.[index] <- struct(FalkanDeviceMemory(block.Bucket, block.Offset, block.Size + blockToMerge.Size), true)
            ...
            blocks.[index] <- struct(FalkanDeviceMemory(block.Bucket, blockToMerge.Offset, block.Size + blockToMerge.Size), true)
