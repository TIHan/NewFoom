[<AutoOpen>]
module Foom.IO.FastPacket.Packet
...
module PacketConstants =
    ...
    [<Literal>]
    let PacketDataSize = 1024

    [<Literal>]
    let PacketPoolAmount = 1024
...
type PacketPool() =
    ...
    let buffer =          ArrayPool.Create(PacketSize, PacketConstants.PacketPoolAmount)
